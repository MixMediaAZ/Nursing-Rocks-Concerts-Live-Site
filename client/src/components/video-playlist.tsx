...
export function VideoPlaylist({
  ...
  useEffect(() => {
    async function fetchVideos() {
      try {
        setLoading(true);
        console.log('[VideoPlaylist] Fetching videos...');
        
        // Fetch videos from server (approval-gated)
        const response = await fetch('/api/videos');
        const data = await response.json();
        console.log('[VideoPlaylist] Fetched data:', data);
        ...
        if (data.success && Array.isArray(data.resources)) {
          ...
          console.log('[VideoPlaylist] Setting videos:', limitedVideos.length, 'videos');
          setVideos(limitedVideos);
        } else {
          console.error('[VideoPlaylist] Failed to fetch videos - no resources in response');
  ...
  // Empty state
  if (videos.length === 0) {
    console.log('[VideoPlaylist] No videos to display - showing empty message');
    return (
  ...
  console.log('[VideoPlaylist] Rendering', videos.length, 'video thumbnails in', layout, 'layout');
