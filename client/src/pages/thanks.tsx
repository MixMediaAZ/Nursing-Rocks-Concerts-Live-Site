...
export default function ThanksPage() {
  ...
  const maxBytes = useMemo(() => 100 * 1024 * 1024, []); // 100MB safety cap
  ...
  const upload = async () => {
    ...
    if (file.size > maxBytes) {
      setError("File too large. Please keep it under 100MB.");
      return;
    }
    ...
    try {
      ...
      // Save submission metadata to database (gracefully handle if endpoint doesn't exist)
      try {
        const submissionRes = await fetch('/api/video-submissions', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            name: meta.name,
        ...
        if (!submissionRes.ok) {
          console.warn('Video submission metadata endpoint not available or failed:', submissionRes.status);
        }
      } catch (err) {
            ...
            <div className="content-box">
              ...
              <div className="mb-6">
                ...
                >
                  ...
                  <div className="text-muted-foreground text-sm">MP4, MOV, AVI, WMV • Max 100MB • Up to 3 minutes</div>
                  <input
                    ref={fileInputRef}
                    type="file"
