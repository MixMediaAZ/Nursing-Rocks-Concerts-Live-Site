/**
 * Standalone verifier runner for Phoenix Children's Jobs
...
async function extractJobDetail(jobId) {
  ...
  try {
    ...
    const applicationUrl = applyButton ? applyButton.getAttribute('href') || url : url;
...
async function runVerification() {
  ...
  for (const jobId of jobsToTest) {
    ...
    if (result.success && result.data) {
      ...
      console.log(`✓ Job ${jobId}: "${result.data.title.substring(0, 40)}..."`);
    } else {
      sampleExtractions.push({
        posting_id: jobId,
